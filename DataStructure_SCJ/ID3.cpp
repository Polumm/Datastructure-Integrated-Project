...
void ID3::setDimension(int d)
{
    this->dimension = d;
}

//x为dimension维的属性向量，y为向量x对应的值  
void ID3::PushData(const int* x, const int y)
{
    List single;
    single.clear();
...
int ID3::Match(const int* x)
{
    return _match(x, root);
}

/// <summary>
/// 匹配输入的类型向量（v）和当前决策树节点（node）的类型
/// </summary>
/// <param name="v"></param>
/// <param name="node"></param>
/// <returns></returns>
int ID3::_match(const int* v, Node* node)
{
    //若节点是叶子节点，函数返回该节点的类型，是或否
    if (node->index < 0)
        return node->type;
    //迭代寻找
    Child& next = node->next;
    //通过v[node->index - 1]获取向量指定维度处的value,再根据value锁定后继节点
